...
const VentaForm = ({ onSubmit }) => {
  ...
  // Recalcular total cuando cambia auto o descuento
  useEffect(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow mb-6">
      <h2 className="text-xl font-bold text-gray-700 mb-4">🛒 Nueva Venta</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div>
          <label className="text-sm text-gray-500 mb-1 block">
            Total (S/) <span className="text-indigo-400 text-xs">(calculado automáticamente)</span>
          </label>
          <input name="total" type="number" value={formData.total} readOnly
            className="border p-2 rounded-lg w-full bg-gray-50 text-indigo-600 font-bold cursor-not-allowed" />
        </div>
      ...
      {/* Resumen */}
      {precioBase > 0 && (
        <div className="mt-4 bg-indigo-50 rounded-xl p-4 flex flex-wrap gap-4 text-sm">
